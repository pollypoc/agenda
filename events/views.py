...
def split_date(string_date):
    """Transforma a data em YYYY-MM-DD em uma tupla de três valores para utilizar
    na visão de eventos de um determinado dia."""
...
def delete(request, id: int):
    ...
    if event.delete():
        return redirect('agenda-events', year=year, month=month, day=day)
    else:
        return server_error(request, 'ops_500.html')

def edit(request):
    """Edita o conteúdo de um evento, recebendo os dados enviados pelo
    formulário, validando e populando em evento já existente."""
    ...
    if form.is_valid():
        ...
        bad_request(request, None, 'ops_400.html')
...
def show(request, id: int):
    ...
    context={
        ...
        'hide_new_button':True,
        'prioriries': Event.priorities_list,
        'today': localdate(),
    }
